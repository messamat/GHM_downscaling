...
def get_continental_hsgrids(continentlist, upapath, pixarea_path, globallakes_fraction_15s, basins_lev12_15s, outputdir,
    ...
    #Clip HR pixel area, global lake fraction, and level 12 basins to the extent of the spatial union of continents in continentlist
    ...
    if not arcpy.Exists(out_pixarea_clip) or overwrite:
        ExtractByMask(in_raster=pixarea_path,
                      ...
                      in_mask_data=cell_pourpoint_path).save(out_pixarea_clip)
    ...
    if not arcpy.Exists(out_pixareafraction_glolakres_clip) or overwrite:
        ExtractByMask(in_raster=globallakes_fraction_15s,
                      ...
                      ).save(out_pixareafraction_glolakres_clip)

    out_baslev12_clip = os.path.join(outputdir,
                                     '{}_lev12_15s.tif'.format(
                                         ''.join(continentlist)))
    if not arcpy.Exists(out_baslev12_clip) or overwrite:
        ExtractByMask(in_raster=basins_lev12_15s,
                      #extraction_area='INSIDE',
                      in_mask_data=cell_pourpoint_path).save(out_baslev12_clip)
